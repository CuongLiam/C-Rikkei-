...
void editStudent(){
    ...
    // Update phone number with duplicate checking and proper validation
    char newSDT[20];
    ...
    if(strlen(newSDT) > 0) {
        int valid = 0;
        while(!valid) {
            if(strlen(newSDT) < 8 || strlen(newSDT) > 11) {
                printf("Error: Phone number must be 8-11 digits. Please re-enter: ");
                fgets(newSDT, sizeof(newSDT), stdin);
                newSDT[strcspn(newSDT, "\n")] = '\0';
                
                if(strlen(newSDT) == 0) break; // keep unchanged if empty input
                continue;
            }
            //ctype.h
            int i, allDigits = 1;
            for(i = 0; i < strlen(newSDT); i++){
                if(!isdigit(newSDT[i])){
                    allDigits = 0;
                    break;
                }
            ...
            if(!allDigits) {
                printf("Error: Phone number must contain only digits. Please re-enter: ");
                fgets(newSDT, sizeof(newSDT), stdin);
                newSDT[strcspn(newSDT, "\n")] = '\0';
                if(strlen(newSDT) == 0) break;
                continue;
            }
            // Check for duplicate phone number (but not the current record)
            int duplicate = 0;
            FILE *dupFile = fopen(STUDENT_PATH, "rb");
            
            if(dupFile != NULL) {
                fseek(dupFile, 0, SEEK_END);
                long fsize = ftell(dupFile);
                int count = fsize / sizeof(struct Student);
                rewind(dupFile);
                
                struct Student tempStudent;
                for(i = 0; i < count; i++){
                    if(fread(&tempStudent, sizeof(struct Student), 1, dupFile) == 1){
                        if(tempStudent.id != s.id && strcmp(tempStudent.phoneNumber, newSDT) == 0){
                            duplicate = 1;
                            break;
                        }
                    }
...
int isValidEmail(const char *email) {
    ...
    if (!at) return 0;		//contains @ 
    if (at == email) return 0;	// char before @
    
    const char *dot = strchr(at, '.');
    if (!dot) return 0;             // "." after @
    if (*(dot + 1) == '\0') return 0; // char after "."
...
void addTeacher(struct Teacher T[], int *n) {
    ...
    do {
        ...
        if(strlen(tempEmail) > 29){
            printf("Error: Email too long. Please try again.\n");
            emailInvalid = 1;
            continue;
        }
        if(!isValidEmail(tempEmail)) {
            printf("Error: Email is not valid. It should have a proper format (e.g., user@gmail.com).\n");
            emailInvalid = 1;
            continue;
        }
        ...
        if(f != NULL){
            ...
            for (i = 0; i < count; i++){
                if(fread(&tempTeacher, sizeof(struct Teacher), 1, f) == 1){
                    if(strcmp(tempTeacher.email, tempEmail) == 0){
                        printf("Error: Email already exists. Please enter a different email.\n");
                        emailInvalid = 1;
                        break;
                    }
                }
    ...
    // Validate phone number with duplicate check (same as before)
    int duplicate;
    do {
        duplicate = 0;
...
void editTeacher() {
    ...
    //validate email and duplication
    if(strlen(newEmail) > 0) {
    	
        int valid = 0;
        while(!valid) {
            if(strlen(newEmail) > 29) {
                printf("Error: Email too long. Please re-enter: ");
                fgets(newEmail, sizeof(newEmail), stdin);
                newEmail[strcspn(newEmail, "\n")] = '\0';
                
                if(strlen(newEmail) == 0) break; // not changing
                continue;
            }
            if(!isValidEmail(newEmail)) {
                ...
                fgets(newEmail, sizeof(newEmail), stdin);
                newEmail[strcspn(newEmail, "\n")] = '\0';
                if(strlen(newEmail) == 0) break;
                continue;
            }
            // Check for duplicate email (but not the current record)
            
            FILE *dupFile = fopen(TEACHER_PATH, "rb");
            int duplicate = 0;
            ...
            if(dupFile != NULL) {
                ...
                for(i = 0; i < count; i++){
                	//read
                    if(fread(&tempTeacher, sizeof(struct Teacher), 1, dupFile) == 1){
                        if(tempTeacher.id != t.id && strcmp(tempTeacher.email, newEmail) == 0){
                            duplicate = 1;
                            break;
                        }
                    }
            ...
            if(duplicate) {
                printf("Error: Email already exists. Please re-enter: ");
                fgets(newEmail, sizeof(newEmail), stdin);
                newEmail[strcspn(newEmail, "\n")] = '\0';
                if(strlen(newEmail) == 0) break;
                continue;
    ...
    // Update Phone Number with duplicate check and validation
    char newPhone[20];
    printf("New phone number (press Enter to keep unchanged): ");
    ...
    if(strlen(newPhone) > 0) {
        int valid = 0;
        while(!valid) {
            if(strlen(newPhone) < 8 || strlen(newPhone) > 11) {
                printf("Error: Phone number must be 8-11 digits. Please re-enter: ");
                fgets(newPhone, sizeof(newPhone), stdin);
                newPhone[strcspn(newPhone, "\n")] = '\0';
                
                if(strlen(newPhone) == 0) break;//same thing as email
                continue;
            }
            int i, isAllDigits = 1;
            for(i = 0; i < strlen(newPhone); i++){
            	//ctype.h
            ...
            // Check for duplicate phone number (but not current record)
            int duplicate = 0;
            FILE *dupFile = fopen(TEACHER_PATH, "rb");
            ...
            if(dupFile != NULL){
                ...
                for(i = 0; i < count; i++){
                    if(fread(&tempTeacher, sizeof(struct Teacher), 1, dupFile) == 1){
                        if(tempTeacher.id != t.id && strcmp(tempTeacher.phoneNumber, newPhone) == 0){
                            duplicate = 1;
                            break;
                        }
                    }
